...
export const useGoalStatusUpdater = () => {
    ...
    try {
      ...
      if (goal.challenge_mode === "amount") {
        isCompleted = goal.target_amount
          ? actualAmount >= goal.target_amount
          : false;
      } else if (goal.challenge_mode === "count") {
        isCompleted = goal.target_count
          ? actualCount >= goal.target_count
          : false;
      } else {
        // 'both' - OR 조건
        const amountComplete = goal.target_amount
          ? actualAmount >= goal.target_amount
          : true;
        const countComplete = goal.target_count
          ? actualCount >= goal.target_count
          : true;
  ...
  const checkGoalsByCategory = async (
    userId: string,
    categoryName: string,
    type: "income" | "expense",
  ) => {
