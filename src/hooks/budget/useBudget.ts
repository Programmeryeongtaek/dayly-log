...
export const useBudget = ({ userId, date, month }: UseBudgetProps = {}) => {
  ...
  const { data: transactions = [], isLoading, error } = useQuery({
    ...
    queryFn: async (): Promise<BudgetTransaction[]> => {
      ...
      // 데이터 통합 및 타입 표시
      const incomeTransactions: BudgetTransaction[] = (incomeResult.data || []).map(item => ({
        ...item,
        type: 'income' as const
      }));

      const expenseTransactions: BudgetTransaction[] = (expenseResult.data || []).map(item => ({
        ...item,
  ...
  const categoryTotals: CategoryTotal[] = useMemo(() => {
    const totalsMap = transactions.reduce((acc, transaction) => {
  ...
  const chartData = useMemo(() => {
    ...
    const expenseData = Array.from(expenseMap.entries()).map(([name, value]) => ({
      name, value
    }));
  ...
  const budgetItems = useMemo(() => {
    return transactions.map(transaction => ({
      ...
      categoryType: transaction.category?.type?.includes('fixed') ? 'fixed' as const : 'variable' as const,
    }));
  }, [transactions]);
