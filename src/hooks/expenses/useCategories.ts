...
import { Category, CategoryFormData } from '@/types/expenses';
...
export const useCategories = (userId?: string) => {
  ...
  const { fixedCategories, variableCategories } = useMemo(() => ({
    fixedCategories: categories.filter(cat => cat.type === 'fixed'),
    variableCategories: categories.filter(cat => cat.type === 'variable'),
  }), [categories]);
