...
import { DomainStats, DomainType, NeighborInfo, NeighborPost } from '@/types/my';
...
// 타입 가드 함수들
const isValidCategory = (category: unknown): category is { name: string; display_name: string } => {
...
export const useNeighborDetailProfile = (neighborId: string) => {
  ...
  const totalPosts = data ? Object.values(data.domainStats).reduce((sum, count) => sum + count, 0) : 0;
