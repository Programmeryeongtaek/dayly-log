...
export const getQuestionVisibilityStatus = (is_public: boolean, is_neighbor_visible: boolean): string => {
  if (is_public && is_neighbor_visible) return '🌍';
  if (is_public && !is_neighbor_visible) return '👤';
  return '🔒';
};

export const getQuestionVisibilityLabel = (is_public: boolean, is_neighbor_visible: boolean): string => {
